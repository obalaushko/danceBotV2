    ...
    user: {
        ...
        notification: {
            main: (user: IUser | null) => {
                const defaultText =
                    'Ви можете керувати сповіщеннями щоб дізнаватися про закінчення абонемента.\n';
                if (!user) return MSG.errors.unknownError;

                return user.notifications
                    ? `${defaultText}\nУ вас увімкнені сповіщення про закінчення абонемента, вимкніть якщо більше не хочете отримувати сповіщень.`
                    : `${defaultText}\nУ вас вимкнуті сповіщення про закінчення абонемента, увімкніть якщо хочете отримати сповіщення коли абонемент підходить до кінця.`;
            },
