...
const connectDb = async (retryCount = 0) => {
    try {
        ...
        dbConnected = false;
        LOGGER.error(`[connectDb][DB connection failed on attempt ${retryCount + 1}]`, {
            metadata: { error: error.message, stack: error.stack.toString() },
        });

        if (retryCount < MAX_RETRIES) {
            LOGGER.info(`[connectDb][Retrying in ${RETRY_INTERVAL / 1000} seconds...]`, { metadata: '' });


            setTimeout(() => connectDb(retryCount + 1), RETRY_INTERVAL);
        } else {
            LOGGER.error(`[connectDb][Max retries reached, connection failed]`, {
                metadata: { error: error.message, stack: error.stack.toString() },
            });
        }
